...
class AllReducer():
    ...
    def _sparse_allreduce(self, name, tensor, selected_tensor, original_shape, topk_indexes=None):
        ...
        if self._compression.name in ['topkA', 'topkA2', "topkA_stable"]:
    ...
    def run(self, new_tensor):
        ...
        elif self._sparse and (self._compression.name == 'oktopk' or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['oktopk'])):
            cstime = time.time()
        ...
        elif self._sparse and (self._compression.name == 'topkAopt' or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['topkAopt'])):
        ...
        elif self._sparse and ((self._compression.name == 'topkSA') or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['topkSA'])):
            cstime = time.time()
            tensor_size = torch.numel(new_tensor.data)
            density = self.get_current_density()
        ...
        elif self._sparse and ((self._compression.name in ['topkA', 'topkA2', 'gtopk']) or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['topkA', 'topkA2', 'gtopk'])):
            density = self.get_current_density()
        ...
        elif self._sparse and (self._compression.name == 'gaussiank' or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['gaussiank'])):
            cstime = time.time()
            density = self.get_current_density()
        ...
        elif self._sparse and (self._compression.name == 'gaussiankconcat' or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['gaussiankconcat'])):
            density = self.get_current_density()
            indexes, values = self._compression.compress(tensor=new_tensor, name=new_name, ratio=density)
            local_topk_indexes = indexes.cpu().numpy().astype(np.int32)
            local_topk_values = values.cpu().numpy().astype(np.float32)
        ...
        elif self._sparse and (self._compression.name == 'gaussiankSA' or (self._compression.name == 'topkA_stable' and self._allreduce_counter[new_name] <= self.stable_topk_threshold and self.stable_topk_warmup_method in ['gaussiankSA'])):
            density = self.get_current_density()
